...
IRRenderer::render_node(ForNode *node) {
  ...
  auto *loop_block = llvm::BasicBlock::Create(llvm_context,
                                              "loop", func);
  ...
  auto *next_var = builder.CreateFAdd(current_var, step_value, "nextvar");
  builder.CreateStore(next_var, alloca);

  end_condition = builder.CreateFCmpONE(end_condition, zero, "loopcond");

  auto *after_block = llvm::BasicBlock::Create(llvm_context,
                                               "afterloop", func);
